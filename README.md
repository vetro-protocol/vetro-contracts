...
**VUSD is a fully collateralized, yield-generating, USD-pegged stablecoin built for DeFi.**

Version 2.0 introduces a modular and robust architecture that separates user interactions, treasury management, and token logic into distinct, auditable contracts. By integrating with premier yield protocols like **Aave, Compound, and Morpho**, VUSD's collateral is put to work, generating passive yield for the protocol.
...
-   **Automated Liquidity Provisioning**: A portion of the generated yield can be used to deepen VUSD liquidity on decentralized exchanges.
-   **Modular & Secure Architecture**: Core logic is split into `Gateway`, `Treasury`, and `VUSD` contracts for clarity and security.
...
The VUSD v2.0 system is composed of three core contracts that work in concert to provide a seamless and secure user experience.
...
graph TD
    ...
    subgraph " "
        ...
        Gateway -- "mints/burns" --> VUSD[VUSD Token]
    ...
    class Gateway,Treasury,VUSD contract
...
3.  **`VUSD.sol`**: The ERC20 stablecoin contract. It includes `ERC20Permit` for gasless approvals and ensures that only the `Gateway` contract can mint new tokens.

## Governance: The Owner-Keeper Model

VUSD v2.0 employs a two-tiered access control model to enhance security:
...
1.  **Clone the repository:**
    ```bash
    git clone https://github.com/hemilabs/vusd-v2.git
    cd vusd-v2
    ```
...
Security is the highest priority for the VUSD protocol. The contracts have been designed with best practices in mind and include:
